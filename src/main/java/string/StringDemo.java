 ...
 * 创建的字符串对象，当再次使用相同字面量创建字符串时会直接重用常量池的对象而不会再次
 * 创建新对象减少内存开销
 * */
public class StringDemo {
    public static void main(String[] args) {
        ...
        //s2与s1字面量相同，因此重用了上面s1创建的字符串对象

        String s2 = "123abc";
        System.out.println("s2"+s2);

        System.out.println(s1==s2);  //s1==s2说明保存的地址是一样的，使用的同一个对象
         ...
         * 将结果编译到class文件中。这样一来虚拟机运行程序时就不必每次都进行计算了。
         * 下面的代码会被编译器改为：
         * String s4 = "123abc";
         */
